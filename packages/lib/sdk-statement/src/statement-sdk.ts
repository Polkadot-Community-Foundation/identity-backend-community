import type { SizedHex } from '@polkadot-api/substrate-bindings'
import { toHex } from '@polkadot-api/utils'
import { getApi, type RequestFn, type SubscribeFn } from './api'
import { type Statement, statementCodec } from './codec'
import type { StatementEvent, SubmitResult, TopicFilter } from './types'
...
export const createStatementSdk = (req: RequestFn, subscribe: SubscribeFn) => {
  ...
  return {
     ...
     */
    submit: (stmt: Statement): Promise<SubmitResult> => api.submit(toHex(statementCodec.enc(stmt))),
    ...
    getBroadcasts: async (
      ...
      const filter: TopicFilter = topics.length > 0 ? { matchAll: topics } : ANY_FILTER
