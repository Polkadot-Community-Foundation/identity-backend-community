#!/usr/bin/env node
/**
 * Parses effect/Schema's TypeScript source and dumps its public API
 * as a TOON-formatted markdown file.
 *
 * Usage: pnpm exec tsx scripts/dump-schema-api.ts
 * Output: docs/effect-schema-public-api.toon
 */
...
const OUTPUT_PATH = path.join(REPO_ROOT, '.cursor', 'rules', 'effect-schema-public-api.mdc')
...
function extractAPI(sourcePath: string): ExtractedAPI {
  ...
  for (const stmt of sourceFile.statements) {
    ...
    // Handle `export declare namespace Schema { ... }` - extract inner exports
...
function formatAsToon(api: ExtractedAPI): string {
  ...
  const content = `S:\n${entries.join('\n')}\n`
  const frontmatter = `---
description: Effect Schema API v${api.version} — all public exports under the S namespace
alwaysApply: true
---

# effect/Schema v${api.version} — \`import * as S from "effect/Schema"\`
...
function main() {
  ...
  if (!fs.existsSync(SCHEMA_PATH)) {
    console.error(`Schema source not found at: ${SCHEMA_PATH}`)
    process.exit(1)
  }

  const api = extractAPI(SCHEMA_PATH)
